...
uint8 setup_DW1000RSTnIRQ(int _enable, uint8 _module_nb)
{
	...
	else
	{
		...
		{
			//HAL_NVIC_DisableIRQ(DW_RESET1_EXTI_IRQn);    //pin #0 -> EXTI #0
			...
			GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
			HAL_GPIO_Init(DW_RESET1_GPIO_Port, &GPIO_InitStruct);
			HAL_GPIO_WritePin(DW_RESET1_GPIO_Port, DW_RESET1_Pin, GPIO_PIN_SET);
		}
		else
			return FALSE;
	}
...
{
    u8 Rxdata;
    HAL_SPI_TransmitReceive(&DWM1000_SPI,&TxData,&Rxdata,1, HAL_MAX_DELAY);       
 	return Rxdata;          		 	
}
...
u8 dwm_readbyte(unsigned char regadr, uint8 _nb)
{
	...
	dwm_spi_readwritebyte(regadr | 0x80);	  //0x80:R/W bit(0->write 1->read)
	reg_data = dwm_spi_readwritebyte(0xff);
...
u8 dwm_writebyte(unsigned char regadr, unsigned char val, uint8 _nb)
{
	...
	dwm_spi_readwritebyte(regadr);
	dwm_spi_readwritebyte(val);
