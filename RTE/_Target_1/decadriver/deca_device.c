...
uint16 index_pdw1000local = 0;    // use for Decawave spi deriver (user code)
...
int dwt_setlocaldataptr(unsigned int index)
{
	...
	/* use for Decawave spi deriver (user code) */
	index_pdw1000local = index;
...
void dwt_isr(void)
{
	...
	uint32 mask = dwt_read32bitreg(SYS_MASK_ID);    // Read mask register 32bits

    // Handle RX good frame event
    if(status & SYS_STATUS_RXFCG)
    {
		if(mask & SYS_MASK_MRXFCG)
		{
			uint16 finfo16;
			uint16 len;
    ...
    {
		if(mask & (SYS_MASK_MRXRFTO | SYS_MASK_MRXPTO))
		{
	...
	// Handle frame SFD timeout events
//    if(status & SYS_STATUS_RXSFDTO)
//    {
//		if(mask & SYS_MASK_MRXSFDTO)
//		{
//			dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXSFDTO); // Clear SFD timeout event bits
//			
//			pdw1000local->wait4resp = 0;
//			
//			// Because of an issue with receiver restart after error conditions, an RX reset must be applied after any error or timeout event to ensure
...
//	}
    ...
    {
		if(mask & (SYS_MASK_MRXPHE | SYS_MASK_MRXFCE | SYS_MASK_MRXRFSL | SYS_MASK_MRXSFDTO | SYS_MASK_MAFFREJ | SYS_MASK_MLDEERR))
		{
...
float dwt_convertrawtemperature(uint8 raw_temp)
{
    ...
    assert(pdw1000local->otp_mask & DWT_READ_OTP_TMP);
#endif
    // the User Manual formula is: Temperature (캜) = ( (SAR_LTEMP ?OTP_READ(Vtemp @ 23캜) ) x 1.14) + 23
